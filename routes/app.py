...
def investment_prediction():
    ...
    if request.method == 'POST':
        try:
            ...
            # Make prediction
            from prediction import predict_investment_profitability
...
def get_company_comparison_updates(id):
    ...
    from prediction import compare_companies
...
@app.route('/compare_all_companies')
@login_required
def compare_all_companies():
    from analyzer import compare_all_companies as compare_logics
    companies = Company.query.all()
    results = compare_logics(companies)
    if not results:
        flash('Not enough CSV data. Please wait for the daily update or initialize the data.', 'warning')
        return redirect(url_for('dashboard'))
    return render_template('compare_all.html', results=results['rankings'], recommendation=results['recommendation'])
