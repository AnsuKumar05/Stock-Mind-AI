...
def predict():
    ...
    if cached_result:
        return jsonify({"status": "completed", "result": cached_result})
    processed = preprocess(input_data)
    executor.submit(background_prediction, cache_key, processed)

    return jsonify({"status": "processing", "message": "Prediction is running. Please check again soon."})
...
def login():
    if request.method == 'POST':
        ...
        if user:
            ...
            flash('Welcome! Your account has been created automatically.', 'success')
            return redirect(url_for('dashboard'))
...
@app.route('/prediction/download/<int:prediction_id>')
@login_required
def download_prediction(prediction_id):
    ...
    pdf.cell(0, 10, f"Prediction Report - ID {prediction.id}", ln=True)

    pdf.set_font("Arial", size=12)
    pdf.cell(0, 10, f"Model: {prediction.model_type}", ln=True)
    pdf.cell(0, 10, f"Price: {prediction.predicted_price}", ln=True)
    pdf.cell(0, 10, f"Confidence: {prediction.confidence_score}", ln=True)
    pdf.cell(0, 10, f"Date: {prediction.created_at}", ln=True)
    pdf.multi_cell(0, 10, f"Prediction Data:\n{prediction.prediction_data}")

    # Output PDF as a Flask response
    response = make_response(pdf.output(dest='S').encode('latin-1'))
    response.headers['Content-Type'] = 'application/pdf'
    response.headers['Content-Disposition'] = f'attachment; filename=prediction_{prediction_id}.pdf'
    return response
