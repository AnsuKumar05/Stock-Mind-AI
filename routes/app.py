...
from routes.ml_predictor import StockPredictor
from routes.extensions import db
from routes.models import User, Upload, Prediction, Company, InvestmentPrediction, CompanyComparison
from concurrent.futures import ThreadPoolExecutor
from flask_caching import Cache
from routes.utils import convert_numpy_types, serialize_prediction_data, generate_prediction_pdf
import time
import json
from routes.seed_companies import initialize_companies
...
import atexit
load_dotenv()
from routes.seed_companies import update_company_data  # make sure your update function is imported


app = Flask(__name__, template_folder='../templates', static_folder='../static', instance_path=os.path.abspath(os.path.join(os.path.dirname(__file__), '../database')))
logging.basicConfig(level=logging.INFO)
# Mute external massive debugging logs from flooding the console
...
def delete_prediction(id):
    from routes.models import Prediction
...
def delete_investment(id):
    from routes.models import InvestmentPrediction
...
def delete_comparison(id):
    from routes.models import CompanyComparison
...
def investment_prediction():
    ...
    if request.method == 'POST':
        try:
            ...
            # Make prediction
            from routes.prediction import predict_investment_profitability
            results = predict_investment_profitability(
            ...
            if results['success']:
                ...
                return render_template('investment_prediction.html', 
                       ...
                       investment_id=investment_pred.id)
...
@app.route('/api/investment_prediction_details/<int:id>', methods=['GET'])
@login_required
def get_investment_prediction_updates(id):
    investment = InvestmentPrediction.query.get_or_404(id)
    company = investment.company
    company_data = {
        'name': company.name,
        'symbol': company.symbol,
    ...
    from routes.prediction import predict_investment_profitability
    results = predict_investment_profitability(
        company_data, investment.investment_amount, investment.time_horizon
    )
    if results.get('success'):
        converted = convert_numpy_types(results)
        # Update db record
        investment.predicted_return = converted['predicted_return']
        investment.predicted_profit = converted['predicted_profit']
        investment.profit_percentage = converted['profit_percentage']
        investment.is_profitable = converted['is_profitable']
        investment.confidence_score = converted['confidence_score']
        investment.prediction_details = serialize_prediction_data(converted['prediction_details'])
        db.session.commit()
...
@app.route('/api/company_comparison_details/<int:id>', methods=['GET'])
@login_required
def get_company_comparison_updates(id):
    comparison = CompanyComparison.query.get_or_404(id)
    company1 = comparison.company1
    company2 = comparison.company2
    company1_data = {
        'name': company1.name,
        'symbol': company1.symbol,
    ...
    from routes.prediction import compare_companies
    results = compare_companies(company1_data, company2_data, comparison.time_horizon)
    if results.get('success'):
        converted = convert_numpy_types(results)
        # Update db record
        comparison.company1_predicted_return = converted['company1_predicted_return']
        comparison.company2_predicted_return = converted['company2_predicted_return']
        comparison.recommended_company_id = company1.id if converted['recommended_company'] == 1 else company2.id
        comparison.confidence_score = converted['confidence_score']
        comparison.comparison_details = serialize_prediction_data(converted['comparison_details'])
        db.session.commit()
        response = jsonify(converted)
        response.headers['Cache-Control'] = 'no-cache, no-store, must-revalidate'
        response.headers['Pragma'] = 'no-cache'
...
def market_data():
    """API endpoint to get the latest 1-day stock data for display"""
    from routes.models import DailyStockData
    today_data = DailyStockData.query.all()
    data = []
    for d in today_data:
        data.append({
...
@login_required
def compare_all():
    """Graph based comparison feature for all companies."""
    companies = Company.query.all()
    return render_template('compare_all.html', companies=companies)

@app.route('/api/compare_all_details', methods=['GET'])
@login_required
def get_compare_all_details():
    """Live JSON feed for the Chart.js sync."""
    companies = Company.query.all()
    data = [{
        'id': c.id,
        'name': c.name,
