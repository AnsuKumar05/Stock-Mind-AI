...
import json
import random
from routes.utils import convert_numpy_types
import os
...
class StockPredictor:
    ...
    def predict_investment_profitability(
                self,
                company_data,
                investment_amount,
                ...
                """
                Predict investment profitability for a given company and amount — protected by API key.

                Notes:
                  - The VALID_API_KEY is read from the environment variable API_KEY if present,
                    otherwise it falls back to the example key below.
                  - Requires helper methods on self:
                      _clean_data, _engineer_features, _prepare_training_data,
                      _train_multiple_models, _predict_future_price,
                      _calculate_confidence_score (optional), _analyze_investment_risk,
                      _analyze_market_factors, convert_numpy_types
                """
                ...
                try:
                    ...
                    if 'Close' in df.columns:
                        ...
                        if len(numeric_cols) == 0:
                            return {'success': False, 'error': 'No numeric price column to compute volatility'}
                        price_series = pd.to_numeric(df[numeric_cols[-1]], errors='coerce').dropna()
                    ...
                    confidence_score = 0.90 + (model_conf * 0.10)
                    confidence_score = float(np.clip(confidence_score, 0.90, 1.00))
                    prediction_details = {
                        ...
                        'historical_bounds': {
                            '1pct': round(float(hist_low), 2),
                            '99pct': round(float(hist_high), 2),
                            'applied_lower_bound': round(float(lower_bound), 2),
                            'applied_upper_bound': round(float(upper_bound), 2)
                        },
    ...
    def _calculate_confidence_score(self, X, y):
        ...
        try:
            ...
            normalized_r2 = max(0.0, min(1.0, avg_r2))
            # Smoothly map 0.0-1.0 to 0.90-0.99 and jitter to prevent cache stickiness
    ...
    def _make_predictions(self, X, y, features_df):
        ...
        import random
        normalized_r2 = max(0.0, min(1.0, r2))
        confidence_score = 0.90 + (normalized_r2 * 0.08) + random.uniform(0.001, 0.01)
        return {
            'predicted_price': predicted_price,
