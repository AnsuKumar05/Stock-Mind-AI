...
def run_daily_update():
    with app.app_context():
        try:
            update_company_data()
            app.logger.info("Daily stock data updated and old data cleaned up successfully.")
        except Exception as e:
            app.logger.error(f"Error updating daily stock data: {e}")

scheduler = BackgroundScheduler()
# Scheduled to run once every day at midnight
scheduler.add_job(func=run_daily_update, trigger="cron", hour=0, minute=0)
scheduler.start()
