...
def generate_prediction_pdf(prediction_data, prediction_type="stock_prediction"):
    ...
    title_map = {
        ...
        "compare_all": "Global Market Network Report"
    }
    ...
    elif prediction_type == "compare_all":
        content.extend(_add_compare_all_content(prediction_data, styles, heading_style))
...
def _add_compare_all_content(data, styles, heading_style):
    content = []
    content.append(Paragraph("Market Top Performing Companies Overview", heading_style))
    
    table_data = [
        ["Symbol", "Company Name", "Price (INR)", "Market Cap (Bn)", "Sector"]
    ]
    
    companies_list = data.get("companies", [])
    # Render all companies spanning multiple pages
    sorted_companies = sorted(companies_list, key=lambda x: x.get('market_cap', 0), reverse=True)

    for c in sorted_companies:
        mcap_val = c.get('market_cap', 0)
        mcap_bn = mcap_val / 1_000_000_000 if mcap_val else 0

        table_data.append([
            c.get("symbol", "N/A"),
            c.get("name", "N/A")[:18] + ".." if len(c.get("name", "")) > 18 else c.get("name", "N/A"),
            f"Rs. {c.get('current_price', 0):,.2f}",
            f"{mcap_bn:,.2f}",
            c.get("sector", "N/A")
        ])
    ...
    # Expand column widths horizontally to prevent visual clipping of large numbers
    table = Table(table_data, hAlign="CENTER", colWidths=[1.1*inch, 1.9*inch, 1.3*inch, 1.5*inch, 1.4*inch])
    _apply_table_style(table, header_color=colors.purple)
