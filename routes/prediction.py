...
def predict_stock(symbol):
    ...
    try:
        ...
        raw_confidence = r2_score if r2_score > 0 else 0.5
        confidence = min(0.99, max(0.5, raw_confidence))
...
def compute_investment(company_data, investment_amount, time_horizon):
    ...
    return {
        ...
        'confidence_score': res.get('confidence', 0.85),
...
def compare_companies(company1_data, company2_data, time_horizon):
    ...
    adjustment = min(0.05, diff / 100.0)
    conf = max(0.85, min(1.0, raw_conf + adjustment))
